...
function KattEvent.new(piped)
  ...
  local event = {
    ...
    addQueue = {},
    removeQueue = {},
    piped = env_type(piped) == "boolean" and piped or false,
  }
...
function KattEvent:register(func, name)
  if env_type(func) ~= "function" then error('argument "func" must be a function.', 2) end
  if name ~= nil and env_type(name) ~= "string" then error('argument "name" must be a string or nil.', 2) end
...
local function eventifyTable(tbl)
  return setmetatable({}, {
    __index = function(t, i)
      if env_type(i) == "string" then
        local eStr = i:upper()
        if env_type(tbl[eStr]) == "Event" then
          return tbl[eStr]
        end
      end
      return tbl[i]
    ...
    __newindex = function(t, i, v)
      if env_type(i) == "string" then
        local eStr = i:upper()
        if env_type(v) == "Event" then
          tbl[eStr] = v
          return
        elseif env_type(tbl[eStr]) == "Event" then
          if env_type(v) ~= "function" then error(("Cannot register non-functions to Event %s."):format(eStr)
            , 2) end
          tbl[eStr]:register(v)
          return
        end
